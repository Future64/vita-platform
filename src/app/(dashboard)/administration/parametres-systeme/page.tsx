...
import { getProposalsForParameter, getParameterProposals } from "@/lib/mockProposals";
...
export default function ParametresSystemePage() {
  ...
  // Get parameter proposals from Agora
  const parameterProposals = useMemo(() => getParameterProposals(), []);
  ...
  const immutableParams = PARAMETERS_BY_CATEGORY.immuable;
  ...
  return (
    <>
      ...
      <div className="mb-4 md:mb-6">
        ...
        <Link
          href="/codex/parametres-systeme"
          className="text-xs text-violet-500 hover:text-violet-400 flex items-center gap-1 mt-2 transition-colors"
        >
          <ExternalLink className="h-3 w-3" />
          Voir la documentation complete dans le Codex
        </Link>
      </div>
        ...
        <Card className="mb-4 md:mb-6">
          <CardHeader>
            ...
            <Badge variant="orange" className="text-xs">
              {parameterProposals.length} proposition{parameterProposals.length > 1 ? "s" : ""} en cours
            </Badge>
          </CardHeader>
          <CardContent className="space-y-3">
            {parameterProposals.map((proposal) => (
              <div
                key={proposal.id}
              ...
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    ...
                    <Badge
                      variant={proposal.statusColor as "green" | "orange" | "cyan"}
                      className="text-xs"
                    >
                  ...
                  {proposal.parameterProposal && (
                    <div className="text-xs text-[var(--text-muted)]">
                      Parametre : {proposal.parameterProposal.parameterName}
                    </div>
                  )}
                </div>
                <Link href={`/agora/${proposal.id}`}>
        ...
        >
          <div className="p-4">
            <button
              ...
              onClick={() => setShowImmutable(!showImmutable)}
            >
              <div className="flex items-center gap-2">
                <Lock className="h-4 w-4 text-red-400" />
                <h2 className="text-lg font-semibold text-[var(--text-primary)]">
                  Parametres immuables
                </h2>
                <Badge variant="red" className="text-xs">
                  {immutableParams.length} constitutionnels
                </Badge>
              </div>
              {showImmutable ? (
            ...
            {showImmutable && (
              <div className="mt-4">
                <p className="text-sm text-[var(--text-muted)] mb-4">
                  Ces parametres sont les fondements constitutionnels de VITA et ne peuvent pas etre modifies.
                </p>

                <div className="space-y-3">
                  {immutableParams.map((param) => (
                    <div
                      key={param.id}
